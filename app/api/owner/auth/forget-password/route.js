...
export const POST = async (request) => {
    ...
    const user = await ownerModal.findOne({ email });
    if (!user) return failedResponse("user not found");
    const token = JWT.sign({ email: user?.email, id: user?.id }, process.env.JWT_SECRET, { expiresIn: '1h' });

    const payload = {
        link: `http://localhost:3000/o/forget-password?token=${token}`,
        expiry: "1 hour"
    }
...
export const PATCH = async (request) => {
    ...
    await ownerModal.findOneAndUpdate({ _id: payload?.id, email: payload?.email }, { password: newHashedPassword });
